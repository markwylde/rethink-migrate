const fs = require('ts-fs-promise')
...
let logLevel, autoToArray, requiresWait
...
function getConfig (root) {
  var config = Object.assign({}, {
    ...
    ssl: nconf.get('ssl')
  }, require(path.join(root, 'config/migrate.js')))
  console.log(config)
...
var rxMigrationFileA = /^\d{14}-.*\.js$/;
var rxMigrationFileB = /^\d{10}-.*\.js$/;
function filterMigrationFiles(file) {
  console.log(file)
  return file.match(rxMigrationFileA) || file.match(rxMigrationFileB);
}
